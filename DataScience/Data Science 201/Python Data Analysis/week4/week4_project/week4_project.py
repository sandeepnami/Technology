...
def filter_by_year(statistics, year, yearid):
    ...
    for player in statistics:
        if int(player[yearid]) == year:
            year_data.append(player)
    return year_data
...
def top_player_ids(info, statistics, formula, numplayers):
    ...
    compute_stats = []
    for player in statistics:
        compute_stats.append((player[info['playerid']], formula(info, player)))
    compute_stats.sort(key=lambda x: x[1], reverse=True)
    return compute_stats[:numplayers]
...
def lookup_player_names(info, top_ids_and_stats):
    ...
    """
    master_statistics_list_dict = read_csv_as_nested_dict(info["masterfile"],info['playerid'],
                                                         info['separator'],info['quote'])
    stats_formatted = []
    for player in top_ids_and_stats:
        if player[0] in master_statistics_list_dict:
            player_name = master_statistics_list_dict[player[0]][info['firstname']] + ' '\
                            + master_statistics_list_dict[player[0]][info['lastname']]
            player_stats = f"{player[1]:.3f} --- {player_name}"    
            stats_formatted.append(player_stats)
...
def compute_top_stats_year(info, formula, numplayers, year):
    ...
    """
    batting_statistics_list_dict = read_csv_as_list_dict(info["battingfile"],
                                                         info['separator'],info['quote'])
    statistics = filter_by_year(batting_statistics_list_dict, year, info['yearid'])
    compute_stats = top_player_ids(info, statistics, formula, numplayers)
    compute_stats_name = lookup_player_names(info, compute_stats)
    return compute_stats_name
...
def aggregate_by_player_id(statistics, playerid, fields):
    ...
    for player in statistics:
        for field in fields:
            if player[playerid] in aggregate_stats:                  
                if field in aggregate_stats[player[playerid]]:
                    aggregate_stats[player[playerid]][field] += int(player[field])
                else:
                    aggregate_stats[player[playerid]][field] = int(player[field])
            else:
                aggregate_stats[player[playerid]] = {}
                aggregate_stats[player[playerid]] [playerid] = player[playerid]
                aggregate_stats[player[playerid]][field] = int(player[field])
...
def compute_top_stats_career(info, formula, numplayers):
    ...
    batting_statistics_list_dict = read_csv_as_list_dict(info["battingfile"],
                                                         info['separator'],info['quote'])
    aggregate_stats = aggregate_by_player_id(batting_statistics_list_dict, 
                                        info['playerid'], info['battingfields'])
    aggregate_stats_list = []
    for key,player in aggregate_stats.items():
        player[info['playerid']] = key
        aggregate_stats_list.append(player)            
    compute_stats = top_player_ids(info, aggregate_stats_list, formula, numplayers)
    compute_stats_name = lookup_player_names(info, compute_stats)
    return compute_stats_name
