       IDENTIFICATION DIVISION.
       PROGRAM-ID. SORTINGSTUDENTS.

       ENVIRONMENT DIVISION.
       INPUT-OUTPUT SECTION.
       FILE-CONTROL.
	   SELECT STUDENTFILE ASSIGN TO "STUDENTS.DAT"
        FILE STATUS IS FILE-CHECK-KEY
		ORGANIZATION IS LINE SEQUENTIAL.
		
       SELECT SORTEDFILE ASSIGN TO "NEWSTUDENTS.DAT"
        ORGANIZATION IS LINE SEQUENTIAL.
		
       SELECT WORKFILE ASSIGN TO "WORK.TMP".
	               
       DATA DIVISION.
       FILE SECTION.
	   FD STUDENTFILE.
	   01 STUDENTDETAILS     PIC X(40).
	        	
       FD SORTEDFILE.
	   01 SORTDETAILS        PIC X(40).
          88 SORTEDFILEEOF      VALUE HIGH-VALUES.
      * DEFINE A WORKFILE FOR SORTING USING SD									
       SD WORKFILE.
       01 WORKREC.
          	02 FILLER        PIC 9(7).
			02 WSTUDENTLNAME PIC X(10).
			02 WSTUDENTFNAME PIC X(10).
			02 FILLER        PIC X(9).
		    02 WMAJOR        PIC X(3).		  
            02 FILLER        PIC X.	   
   
       WORKING-STORAGE SECTION.
       01  WS-WORKING-STORAGE.
           05 FILLER      PIC X(27) VALUE 
		      'WORKING STORAGE STARTS HERE'.    
   
	   01  WS-WORK-AREAS.
	       05  FILE-CHECK-KEY   PIC X(2).
		 
       PROCEDURE DIVISION.
       0100-READ-EMPLOYEES.

		   OPEN INPUT STUDENTFILE.
		   				
		   SORT WORKFILE ON ASCENDING KEY WMAJOR, WSTUDENTLNAME,
              WSTUDENTFNAME		  
		      USING STUDENTFILE GIVING SORTEDFILE.

              OPEN INPUT SORTEDFILE.
              PERFORM UNTIL SORTEDFILEEOF
                 READ SORTEDFILE 
                    AT END SET SORTEDFILEEOF TO TRUE                
                 DISPLAY SORTDETAILS
               END-PERFORM.
              CLOSE SORTEDFILE.
		 
		   PERFORM 9000-END-PROGRAM.
		   
	   0100-END.
	   
	   
	   9000-END-PROGRAM.
           CLOSE STUDENTFILE.	
           STOP RUN.
        
          END PROGRAM SORTINGSTUDENTS.
