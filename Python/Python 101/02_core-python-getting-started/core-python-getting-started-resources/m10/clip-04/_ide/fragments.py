...
snapshot_filepaths = [
    steps_dirpath / "airtravel_03.py",
    steps_dirpath / "airtravel_04.py",
    steps_dirpath / "airtravel_05.py",
]
...
def prepare():
    ...
    with Idea.localhost(14415) as idea:
        ...
        virtual_environment_python = expanduser("~/.virtualenvs/airtravel/bin/python")
...
def fragment_0a(rate):
    prng = random.Random(0)
    logger.info("fragment_0a delegate rate = %f", rate)
    with Idea.localhost(14415, prng=prng) as idea:
...
def fragment_4(rate):
    ...
    with Idea.localhost(14415, prng=prng) as idea:
        idea.type_text("f.number()", rate=rate)
        idea.type_execute_statement(rate=rate)
